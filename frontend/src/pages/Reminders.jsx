  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      reminders.forEach((r) => {
        ...
        reminderTime.setHours(r.time.hour);
        reminderTime.setMinutes(r.time.minute);

        const diff = reminderTime - now;

        if (diff < before * 60000 && diff > 0) {
          showNotification(
            r.title,
            r.description || "Reminder time"
          );
        }
      });
    }, 60000);
        ...
        {reminders.length === 0 ? (
          ...
          reminders.map((r) => (
            <div className="card" key={r._id}>
              ...
              <div className="time">
                {r.date?.slice(0, 10)} - {formatTime(r.time)}
              </div>
            </div>
          ))
        )}
